...
## 1. Версия Node.js (совпадение с локальной)

Чтобы и сборка, и serverless-функции (API) работали на Node 20, как локально:

- В **Vercel Dashboard** → проект → **Settings** → **General** (или **Build and Deployment**) найдите **Node.js Version** и выберите **20.x**.

В репозитории уже задано `"engines": { "node": "20.x" }` в `package.json`; Vercel может использовать это для сборки. Явный выбор **20.x** в настройках проекта гарантирует, что и функции (`api/index.ts`) запускаются на Node 20, а не на дефолтной 24.x.
